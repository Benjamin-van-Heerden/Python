...
def coefficientFactor(partition, num_colors ,factorialTable):
    """
    Computes
    ...
    for n in partition:
        if n in counts:
            ...
            counts[n] = 1
    result = factorial(num_colors, factorialTable)
    for color, frequency in counts.items():
        result //= (color ** frequency) * factorial(frequency, factorialTable)
    return result
...
def solution(w, h, s):
    ...
    # for the formula.
    grid=0
    print(partitionsAndCycleCount(w,factorialTable))
    print(partitionsAndCycleCount(h,factorialTable))
