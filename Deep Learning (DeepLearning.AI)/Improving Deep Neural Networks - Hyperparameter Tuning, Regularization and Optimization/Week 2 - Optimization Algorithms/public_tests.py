...
def update_parameters_with_gd_test(target):
    ...
    for key in params_up.keys():
        assert type(params_up[
                        key]) == np.ndarray, f"Wrong type for {key}. We expected np.ndarray, but got {type(params_up[key])}"
        assert params_up[key].shape == parameters[
            key].shape, f"Wrong shape for {key}. {params_up[key].shape} != {parameters[key].shape}"
        assert np.allclose(params_up[key], expected_output[
...
def update_parameters_with_adam_test(target):
    ...
    for key in parameters.keys():
        ...
        assert parameters[key].shape == parametersi[
